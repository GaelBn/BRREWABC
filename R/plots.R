...
#'
#' @param data a dataframe containing the estimation results (set of particles accepted during iterations)
#' @param prior a list linking model name (character string) to a list describing the prior distribution of each parameter estimated (the same as used for the abcsmc function)
#' @param filename the file name to be used to save the plots (the extension defines the format: pdf or png)
...
plot_abcsmc_res = function(data, prior, filename = "pairplot.png", figtitle = "ABC smc results", colorpal = "GnBu", iter = NA){
    ...
    mycolors = grDevices::colorRampPalette(RColorBrewer::brewer.pal(8, colorpal))(nb.cols)
    ...
    if (length(unique(tmp$model)) >= 1) {
        ...
        for (mm in sort(unique(tmp$model))) {
            ...
            pairplot = GGally::ggpairs(tmpm[c(param_names, "gen")], upper = "blank", ggplot2::aes(fill = gen, colour = gen, alpha=0.8),
                columns = param_names, lower = list(continuous = GGally::wrap("points", alpha = 0.5, size=1.5)),
                title = paste(figtitle, mm, sep=" - ")
             ) + ggplot2::scale_fill_manual(values = mycolors) + ggplot2::scale_colour_manual(values = mycolors) + ggplot2::theme_minimal() #theme_dark()
            if (last_three == "png") {
                newfilename = paste0(substr(filename, 1, nchar(filename)-4), '_',  mm,'.png')
    ...
    for (mm in sort(unique(tmp$model))) {
        for (p in prior[[mm]]) {
            ...
            sub_plot = ggplot2::ggplot(subtmp, ggplot2::aes(x=param, y=gen, fill=gen, color=gen))+
                          ggridges::geom_density_ridges(ggplot2::aes(), scale = 2.0, alpha = .6, bandwidth = (as.double(p[4]) - as.double(p[3]))/100) + ggridges::theme_ridges() +
...
#'
#' @param data a dataframe containing the estimation results (thresholds over iterations)
#' @param nb_threshold number of thresholds used (see the abcsmc function help for more details)
#' @param filename the file name to be used to save the plots (the extension defines the format: pdf or png)
#' @param figtitle the figure title
#' @param colorpal a palette name as used in the RColorBrewer package
#'
